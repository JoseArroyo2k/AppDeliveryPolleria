...
class MundoVerdePage extends StatefulWidget {
  @override
  _MundoVerdePageState createState() => _MundoVerdePageState();
}

class _MundoVerdePageState extends State<MundoVerdePage> {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  List<Map<String, dynamic>> mundoVerdeProducts = [];

  @override
  void initState() {
    super.initState();
    _fetchMundoVerdeProducts();
  }

  void _fetchMundoVerdeProducts() async {
    try {
      QuerySnapshot snapshot = await _firestore
          .collection('Productos')
          .where('Categoria', isEqualTo: 'MundoVerde')
          .get();

      setState(() {
        mundoVerdeProducts = snapshot.docs.map((doc) => doc.data() as Map<String, dynamic>).toList();
      });
    } catch (e) {
      print('Error al obtener productos de la categoría Mundo Verde: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: mundoVerdeProducts.isEmpty
          ? Center(child: CircularProgressIndicator())
          : ListView.builder(
              itemCount: mundoVerdeProducts.length,
              itemBuilder: (context, index) {
                var product = mundoVerdeProducts[index];
                try {
