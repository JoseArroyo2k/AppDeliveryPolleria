...
class GuarnicionesPage extends StatefulWidget {
  @override
  _GuarnicionesPageState createState() => _GuarnicionesPageState();
}

class _GuarnicionesPageState extends State<GuarnicionesPage> {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  List<Map<String, dynamic>> guarnicionesProducts = [];

  @override
  void initState() {
    super.initState();
    _fetchGuarnicionesProducts();
  }

  void _fetchGuarnicionesProducts() async {
    try {
      QuerySnapshot snapshot = await _firestore
          .collection('Productos')
          .where('Categoria', isEqualTo: 'Guarniciones')
          .get();

      setState(() {
        guarnicionesProducts = snapshot.docs.map((doc) => doc.data() as Map<String, dynamic>).toList();
      });
    } catch (e) {
      print('Error al obtener productos de la categoría Guarniciones: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: guarnicionesProducts.isEmpty
          ? Center(child: CircularProgressIndicator())
          : ListView.builder(
              itemCount: guarnicionesProducts.length,
              itemBuilder: (context, index) {
                var product = guarnicionesProducts[index];
                try {
