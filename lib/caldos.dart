...
class CaldosPage extends StatefulWidget {
  @override
  _CaldosPageState createState() => _CaldosPageState();
}

class _CaldosPageState extends State<CaldosPage> {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  List<Map<String, dynamic>> caldosProducts = [];

  @override
  void initState() {
    super.initState();
    _fetchCaldosProducts();
  }

  void _fetchCaldosProducts() async {
    try {
      QuerySnapshot snapshot = await _firestore
          .collection('Productos')
          .where('Categoria', isEqualTo: 'Caldos')
          .get();

      setState(() {
        caldosProducts = snapshot.docs.map((doc) => doc.data() as Map<String, dynamic>).toList();
      });
    } catch (e) {
      print('Error al obtener productos de la categoría Caldos: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: caldosProducts.isEmpty
          ? Center(child: CircularProgressIndicator())
          : ListView.builder(
              itemCount: caldosProducts.length,
              itemBuilder: (context, index) {
                var product = caldosProducts[index];
                try {
